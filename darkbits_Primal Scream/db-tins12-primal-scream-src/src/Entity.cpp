...
int Entity::getGravityDirection() {return mGravityDirection;}
void Entity::toggleGravityDirection(int timer){mGravityDirection*=-1;}
...
void Entity::setInitialPosition(float2 position){
...
bool Entity::touchesDangerousTile()
{
	CollisionRect r = getCollisionRect();
	int x1 = (int)((r.myTopLeft.x + 5.0)/ getRoom()->getTileWidth());
	int y1 = (int)((r.myTopLeft.y + 5.0 )/ getRoom()->getTileHeight());
	int x2 = (int)((r.myBottomRight.x - 5.0) / getRoom()->getTileWidth());
	int y2 = (int)((r.myBottomRight.y - 5.0) / getRoom()->getTileHeight());

	x1 = clamp(x1, 0, getRoom()->getWidthInTiles());
	y1 = clamp(y1, 0, getRoom()->getHeightInTiles());
	x2 = clamp(x2, -1, getRoom()->getWidthInTiles() - 1);
	y2 = clamp(y2, -1, getRoom()->getHeightInTiles() - 1);

	for (int y = y1; y <= y2; y++)
	{
		for (int x = x1; x <= x2; x++)
		{
			if (getRoom()->isDangerous(x, y))
			{
				return true;
			}
		}
